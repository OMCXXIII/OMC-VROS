...
const VoiceCore = {
    ...
    init() {
        ...
        // Auto-restart enquanto escuta estiver ativa
    ...
    toggle() {
        ...
        if (this.isListening) {
            ...
            // Classe no body → pulsa reactor-core via CSS (.jarvis-listening)
            document.body.classList.add('jarvis-listening');

            // HUD: mantém indicador JARVIS ATIVO até desligar
            ...
            // Feedback 3D: núcleo vermelho → ciano
            const core = document.getElementById('singularidade');
            if (core) {
    ...
    processIntent(text) {
        ...
        // Dicionário de intenções (expansível)
        if      (cmd.includes('navegar para biometria'))             navigate('1');
        else if (cmd.includes('navegar para diagnóstico'))           navigate('2');
        else if (cmd.includes('navegar para o centro'))              navigate('3');
        else if (cmd.includes('navegar para nexus'))                 navigate('4');
        else if (cmd.includes('navegar para soberania'))             navigate('5');
        ...
        else if (cmd.includes('salvar memória') || cmd.includes('coagular')) Kernel.save(SYSTEM_STATE.ops.buffer);
        else if (cmd.includes('limpar buffer'))                      { SYSTEM_STATE.ops.buffer = ''; _flushBuffer(); }
            ...
            _flushBuffer();
            updateBioStatus(`JARVIS: DITADO — ${text.slice(0, 40)}${text.length > 40 ? '…' : ''}`);
        }
    ...
    /* ── Exibe transcrição intermediária no monitor 3D ── */
    _showInterim(text) {
        const display = document.getElementById('execution-text');
        if (display) {
...
window.addEventListener('load', () => {
    ...
    if (window.IS_MOBILE) {
        ...
        console.log('[MOBILE] Modo baixo consumo ativado.');
    }

    // Inicializa VoiceCore (standby — aguarda Alt+V ou /jarvis)
    VoiceCore.init();
    ...
    // rAF loop — proximidade quântica (throttle em mobile)
    let lastProx = 0;
